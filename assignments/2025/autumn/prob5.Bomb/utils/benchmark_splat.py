...
def run_benchmark(gen_cmd, baseline_bin, submit_bin, heightmap_arg):
    def run(binary, label):
        gen = subprocess.Popen(gen_cmd, stdout=subprocess.PIPE, text=True)
        try:
            proc = subprocess.run(
                ["./" + binary] + heightmap_arg,
                stdin=gen.stdout,
                capture_output=True,
                text=True,
                check=True,
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: benchmark.py <size> <seed> <baseline_bin> <submit_bin> [heightmap]", file=sys.stderr)
        sys.exit(1)
    ...
    heightmap = sys.argv[5] if len(sys.argv) > 5 else ""
    
    gen_cmd = ["python3", "gen_splat.py", "--size", size, "--seed", seed]
    heightmap_arg = ["--input-heightmap", heightmap] if heightmap else []
    run_benchmark(gen_cmd, baseline_bin, submit_bin, heightmap_arg)
